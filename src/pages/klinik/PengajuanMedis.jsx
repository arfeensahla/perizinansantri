...
import { Send, User, CalendarClock, FileText, AlertCircle, Info, Clock, CheckCircle, Stethoscope, Search, ChevronDown } from 'lucide-react';

const PengajuanMedis = () => {
    // --- State Form ---
    const [santriTerpilih, setSantriTerpilih] = useState('');
    const [santriTerpilihObj, setSantriTerpilihObj] = useState(null); // Menyimpan objek lengkap santri

    // State Khusus Searchable Dropdown
    const [searchSantri, setSearchSantri] = useState('');
    const [isDropdownSantriBuka, setIsDropdownSantriBuka] = useState(false);

    const [jenisIzin, setJenisIzin] = useState(''); // 'RUJUK_INAP_KLINIK' atau 'RAWAT_JALAN_KLINIK'
    const [diagnosis, setDiagnosis] = useState('');

    // State Pendamping (Khusus Rawat Jalan / PP)
    ...
    // --- Data Dummy (Klinik memiliki akses ke seluruh santri) ---
    const daftarSantri = [
        { nis: '260011', nama: 'Ahmad Muzakki', kelas: '7A' },
        { nis: '260032', nama: 'Dimas Anggara', kelas: '7A' },
        { nis: '250114', nama: 'Tariq bin Ziyad', kelas: '8B' },
        { nis: '250188', nama: 'Faisal Rahman', kelas: '8B' },
        { nis: '240089', nama: 'Zaid bin Tsabit', kelas: '9A' },
        { nis: '240092', nama: 'Umar Al-Faruq', kelas: '9C' },
    ];

    // Logika Pencarian Santri
        ...
        <div className="animate-fade-in-down p-2 md:p-6 pb-24 max-w-3xl mx-auto">
            ...
            {isSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden relative">
                    ...
                    {/* Layer transparan untuk menutup dropdown santri jika klik di luar */}
                    {isDropdownSantriBuka && (
                        <div className="fixed inset-0 z-10" onClick={() => setIsDropdownSantriBuka(false)}></div>
                    )}
                    ...
                    <div className="p-6 space-y-6 relative z-20">
                        {/* 1. Pilih Santri (SEARCHABLE DROPDOWN) */}
                        <div>
                            ...
                            <div className="relative">
                                {/* Tombol Pilih (Tampilannya seperti Input) */}
                                <div
                                    onClick={() => setIsDropdownSantriBuka(!isDropdownSantriBuka)}
                                    className={`w-full px-4 py-3 bg-gray-50 border rounded-xl text-sm transition-all cursor-pointer flex justify-between items-center ${isDropdownSantriBuka ? 'border-rose-500 ring-2 ring-rose-100 bg-white' : 'border-gray-200 hover:border-rose-300'}`}
                                >
                                    {santriTerpilihObj ? (
                                        <div className="flex flex-col">
                                            <span className="font-bold text-gray-900">{santriTerpilihObj.nama}</span>
                                            <span className="text-[10px] text-gray-500 uppercase mt-0.5">Kelas {santriTerpilihObj.kelas} • NIS: {santriTerpilihObj.nis}</span>
                                        </div>
                                    ) : (
                                        <span className="text-gray-400">-- Cari Nama / Kelas / Nomor Induk --</span>
                                    )}
                                        ...
                                        <div className="p-2 border-b border-gray-100 bg-gray-50">
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 text-sm"
                                                    autoFocus
                                                />
                                            ...
                                            {santriTampil.length === 0 ? (
                                                ...
                                                santriTampil.map(santri => (
                                                    <div
                                                        key={santri.nis}
                                                        onClick={() => {
                                                            setSantriTerpilih(santri.nis);
                                                            setSantriTerpilihObj(santri);
                                                            setIsDropdownSantriBuka(false);
                                                            setSearchSantri('');
                                                        }}
                                                        className="p-3 hover:bg-rose-50 rounded-lg cursor-pointer transition-colors border-b border-gray-50 last:border-0"
                                                    >
                                                        <div className="font-bold text-gray-800">{santri.nama}</div>
                                                        <div className="text-xs text-gray-500 mt-1">Kelas {santri.kelas} • NIS: {santri.nis}</div>
                                                    </div>
                                                ))
                                            )}
                                        </div>
                        ...
                        {/* 2. Jenis Izin & Alasan (Sama persis dengan Walikelas) */}
                        <div className="border-t border-gray-100 pt-6 relative z-0">
                            <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                                <FileText size={16} className="text-rose-600" /> Kategori Medis & Diagnosis
                            </label>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <label className={`cursor-pointer flex items-center p-4 rounded-xl border-2 transition-all ${jenisIzin === 'RUJUK_INAP_KLINIK' ? 'border-rose-500 bg-rose-50' : 'border-gray-100 bg-white hover:border-rose-200'}`}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-[11px] text-gray-500 mt-1">Santri menginap di Rumah Sakit / Rumah.</div>
                                    </div>
                            ...
                            {/* LOGIKA CONDITIONAL KHUSUS PENDAMPING KLINIK (Jika Rawat Jalan) */}
                            {jenisIzin === 'RAWAT_JALAN_KLINIK' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 animate-fade-in">
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1.5">Nama Petugas Pendamping</label>
                                        <input
                                            required type="text" value={namaPendamping} onChange={(e) => setNamaPendamping(e.target.value)}
                                            placeholder="Contoh: Ustadz Budi (Perawat)"
                                            className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-rose-500 text-sm transition-all"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1.5">Nomor WhatsApp Pendamping</label>
                                        <input
                                            required type="text" value={hpPendamping} onChange={(e) => setHpPendamping(e.target.value)}
                                            placeholder="Contoh: 081234567890"
                                            className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-rose-500 text-sm font-mono transition-all"
                                        />
                                    </div>
                                </div>
                            )}
                            ...
                            <textarea
                                ...
                                rows="3"
                                placeholder="Jelaskan diagnosis medis & Fasilitas Kesehatan Tujuan (Contoh: Gejala Typus, rujuk ke RSUD Majalengka)..."
                                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-rose-500 focus:bg-white text-sm transition-all resize-none"
                            ></textarea>
                        </div>

                        {/* 3. Rentang Waktu Perizinan (Sama persis dengan Walikelas) */}
                        <div className="border-t border-gray-100 pt-6 relative z-0">
                            <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-4">
                                <CalendarClock size={16} className="text-amber-600" /> Jadwal Perizinan Medis
                            </label>
                    ...
                    <div className="bg-gray-50 p-6 border-t border-gray-100 flex items-center justify-between relative z-0">
                        <div className="hidden md:flex items-center gap-2 text-xs text-gray-500">
                            <AlertCircle size={14} /> Pastikan kondisi pasien telah diperiksa oleh perawat jaga.
                        </div>
                        <button
                            type="submit"
                            disabled={isSubmitting || !santriTerpilih}
                            className={`w-full md:w-auto px-8 py-3 rounded-xl font-bold text-sm text-white flex items-center justify-center gap-2 transition-all shadow-sm ${isSubmitting || !santriTerpilih ? 'bg-gray-400 cursor-not-allowed' : 'bg-rose-600 hover:bg-rose-700'
                                }`}
                        >
                            {isSubmitting ? (
