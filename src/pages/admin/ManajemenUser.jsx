...
import { Users, Search, Plus, Edit, Trash2, Shield, KeyRound, X, UploadCloud, Download, FileSpreadsheet, Loader2 } from 'lucide-react';
...
const ManajemenUser = () => {
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            const formattedData = data.map(u => ({
                id: u.id,
                nama: u.nama_lengkap,
                username: u.nama_lengkap.toLowerCase().replace(/\s+/g, '').substring(0, 10),
                role: u.role,
    ...
    return (
        // Menggunakan Fragment (<>) agar Modal bisa keluar dari kotak wrapper animasi
        <>
            <div className="animate-fade-in-down p-2 md:p-6 pb-24 max-w-7xl mx-auto">
                ...
                <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            onClick={fetchUsers}
                            className="bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all shadow-sm hidden md:flex"
                        >
                            Segarkan Data
                        </button>
                        <button
                        ...
                        <table className="w-full text-sm text-left min-w-[600px]">
                            ...
                            <tbody>
                                {isLoading ? (
                                    ...
                                    <tr>
                                        <td colSpan="4" className="px-6 py-8 text-center text-red-500 font-bold">{errorMsg}</td>
                                    </tr>
                                ) : dataTampil.length === 0 ? (
                                    <tr>
                                        <td colSpan="4" className="px-6 py-8 text-center text-gray-500">Tidak ada pengguna yang ditemukan.</td>
                                    </tr>
                                ) : dataTampil.map((user) => (
            ...
            {isModalBuka && (
                <div
                    ...
                    onClick={() => setIsModalBuka(false)} // Menutup saat klik area gelap
                >
                    <div
                        className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-slide-up"
                        onClick={(e) => e.stopPropagation()} // Mencegah klik dari dalam form putih menutup modal
                    >
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <select value={formRole} onChange={(e) => setFormRole(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium">
            ...
            {isModalImportBuka && (
                <div
                    className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-fade-in"
                    onClick={() => setIsModalImportBuka(false)}
                >
                    <div
                        className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-slide-up"
                        ...
                        <div className="p-6">
                            ...
                            <div>
                                ...
                                <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 hover:border-emerald-400 transition-all cursor-pointer group">
                                    ...
                                    <p className="text-xs text-gray-500">Mendukung format .xlsx atau .csv</p>
                                </div>
                            </div>
                        </div>
                        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
