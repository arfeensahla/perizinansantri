...
const MasterData = () => {
    ...
    const [dataKelas] = useState([
        { id: 1, nama: '7A', wali: 'Ust. Fulan', totalSantri: 32 },
        { id: 2, nama: '7B', wali: '-', totalSantri: 30 },
        { id: 3, nama: '8A', wali: 'Ust. Mahmud', totalSantri: 28 },
        { id: 4, nama: '8B', wali: 'Ust. Budi', totalSantri: 34 },
        { id: 5, nama: '9A', wali: 'Ust. Zulfikar', totalSantri: 35 },
    ]);

    // --- Data Dummy Santri ---
    const [dataSantri] = useState([
        { id: 1, nis: '260011', nama: 'Ahmad Muzakki', gender: 'L', kotaAsal: 'Cirebon', kelas: '7A', waliOrtu: 'Bpk. Ridwan', noWa: '081234567890' },
        { id: 2, nis: '260012', nama: 'Faisal Rahman', gender: 'L', kotaAsal: 'Bandung', kelas: '8B', waliOrtu: 'Ibu Nisa', noWa: '081298765432' },
        { id: 3, nis: '260013', nama: 'Zaid bin Tsabit', gender: 'L', kotaAsal: 'Jakarta', kelas: '9A', waliOrtu: 'Bpk. Hasan', noWa: '085612341234' },
        { id: 4, nis: '260014', nama: 'Aisyah Putri', gender: 'P', kotaAsal: 'Tegal', kelas: '7C', waliOrtu: 'Ibu Sarah', noWa: '081345678901' },
        { id: 5, nis: '260015', nama: 'Tariq bin Ziyad', gender: 'L', kotaAsal: 'Kuningan', kelas: '9B', waliOrtu: 'Bpk. Usman', noWa: '087812345678' },
    ]);

    // --- Filter Logika ---
    const santriTampil = dataSantri.filter(s => {
        const matchKata = s.nama.toLowerCase().includes(kataKunciSantri.toLowerCase()) ||
            s.nis.includes(kataKunciSantri) ||
    ...
    return (
        <div className="animate-fade-in-down p-2 md:p-6 pb-24 max-w-7xl mx-auto">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-500 text-sm mt-1">Pusat pengelolaan data referensi kelas dan santri pondok.</p>
            </div>
                    ...
                    <div className="bg-white p-4 rounded-t-2xl border border-gray-200 border-b-0 flex flex-col md:flex-row gap-4 items-center justify-between">
                        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto flex-1">
                            <div className="relative flex-1 md:max-w-xs">
                                <input type="text" placeholder="Cari Nama / NIS / Kota..." value={kataKunciSantri} onChange={(e) => setKataKunciSantri(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm" />
                            ...
                            <table className="w-full text-sm text-left min-w-[850px]">
                                <thead className="text-[11px] text-gray-500 uppercase tracking-wider bg-gray-50 border-b">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4">Kelas & Gender</th>
                                        <th className="px-6 py-4">Kota Asal</th>
                                        <th className="px-6 py-4">Kontak Wali / Ortu</th>
                                        ...
                                        <tr key={santri.id} className="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col items-start gap-1.5">
                                                    ...
                                                    <span className={`text-[11px] font-bold ${santri.gender === 'L' ? 'text-blue-600' : 'text-pink-600'}`}>
                                                        {santri.gender === 'L' ? 'Laki-laki (L)' : 'Perempuan (P)'}
                                                    </span>
                                                </div>
                                            </td>
                                            {/* KOLOM KOTA ASAL (BERDIRI SENDIRI) */}
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="font-semibold text-gray-800">{santri.waliOrtu}</div>
                                                <div className="text-xs text-gray-500 mt-1 flex items-center gap-1.5"><Phone size={12} /> {santri.noWa}</div>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit Santri"><Edit size={16} /></button>
                                ...
                                <tbody>
                                    {kelasTampil.map((kelas) => (
                                        <tr key={kelas.id} className="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-md text-xs font-bold">{kelas.totalSantri} Anak</span>
                                            </td>
                                            <td className="px-6 py-4 font-medium text-gray-700">
                                                {kelas.wali !== '-' ? kelas.wali : <span className="text-red-500 italic text-xs">Belum di-assign di Manajemen User</span>}
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-slide-up">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1.5">NIS</label>
                                    <input type="text" placeholder="Nomor Induk" className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-sm font-mono" />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <input type="text" placeholder="Nama sesuai ijazah" className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-sm" />
                            </div>
                            ...
                            <div className="pt-2 border-t border-gray-100">
                                ...
                                <input type="text" placeholder="Contoh: Bpk. Haryanto" className="w-full px-4 py-2 mb-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-sm" />
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Nomor WhatsApp Wali</label>
                                <input type="text" placeholder="081234..." className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-sm font-mono" />
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-slide-up">
                        ...
                        <div className="p-6">
                            <div className="mb-6">
                                ...
                                <p className="text-xs text-gray-500 mb-3">Kolom wajib diisi: <strong>NIS, Nama, Gender, Asal Kota, Kelas, Nama Wali, No WA</strong>.</p>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-slide-up">
                        ...
                        <div className="p-6">
                            ...
                            <p className="text-[11px] text-gray-500 mt-3 bg-blue-50 p-3 rounded-lg border border-blue-100">
                                💡 <strong>Catatan:</strong> Untuk mengikat Walikelas pada kelas ini, silakan atur melalui halaman <strong>Manajemen User</strong>.
                            </p>
                        </div>
                        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
